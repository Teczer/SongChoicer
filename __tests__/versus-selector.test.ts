...
describe('songFunctions', () => {
  const song1: Song = { id: 1, image: { url: 'url1' }, title: 'Song 1' };
  const song2: Song = { id: 2, image: { url: 'url2' }, title: 'Song 2' };
  const song3: Song = { id: 3, image: { url: 'url3' }, title: 'Song 3' };
  const song4: Song = { id: 4, image: { url: 'url4' }, title: 'Song 4' };
  ...
  describe('findPairWithDesiredIdsAndNoAvoidIds', () => {
    it('should return a pair with desired ids and no avoid ids', () => {
      expect(findPairWithDesiredIdsAndNoAvoidIds([1], [3, 4], allDuels)).toEqual(versus1);
    });

    it('should return undefined if no pair with desired ids and no avoid ids exists', () => {
      expect(findPairWithDesiredIdsAndNoAvoidIds([1, 2], [1, 2], allDuels)).toBeUndefined();
    });
  });

  describe('findFirstDuelsOfASongWithoutAnotherSongId', () => {
    it('should return a pair with desired ids and no avoid ids', () => {
      expect(findFirstDuelsOfASongWithoutAnotherSongId([1], [3, 4], allDuels)).toEqual(versus1);
    });

    it('should return a pair with only desired ids if no pair with desired ids and no avoid ids exists', () => {
      expect(findFirstDuelsOfASongWithoutAnotherSongId([3, 4], [1, 2], allDuels)).toEqual(versus2);
    });

    it('should return a pair with no avoid ids if no pair with desired ids exists', () => {
      expect(findFirstDuelsOfASongWithoutAnotherSongId([], [1, 2], allDuels)).toEqual(versus2);
    });

    it('should return the first pair if no other criteria are met', () => {
      expect(findFirstDuelsOfASongWithoutAnotherSongId([], [], allDuels)).toEqual(versus1);
    });
  });
});
