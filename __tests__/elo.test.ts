...
import { generateAllPossiblePairWithoutDuplicate, generateDuels } from '@/lib/duels';
...
describe('generateDuels', () => {
  ...
  it(`should have all unique possible pair`, () => {
    const allUniqueDuelsPossible = generateAllPossiblePairWithoutDuplicate(songs);
  ...
  it('should distribute song appearances approximately equally', () => {
    const duels = generateDuels(songs);
    const appearanceCount: { [key: number]: number } = countHowMuchTimeThisSoungAppear(duels);
